      ...
      parallel {
        ...
        stage('Quality Tests') {
          steps {
            ...
            sh 'docker tag nodeapp-dev:trunk syedraza325/nodeapp-dev:latest'
            sh 'docker push syedraza325/nodeapp-dev:latest'
          }
        }
      }
      post {
            ...
            steps {
                    retry(3) {
                        timeout(time:10, unit: 'MINUTES') {
                            sh 'docker tag nodeapp-dev:trunk syedraza325/nodeapp-prod:latest'
                            sh 'docker push syedraza325/nodeapp-prod:latest'
                            sh 'docker save syedraza325/nodeapp-prod:latest | gzip > nodeapp-prod-golden.tar.gz'
                        }
                    }
